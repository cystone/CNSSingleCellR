...
write.csv(all.markers, "03_cellIdentify/diff_genes_wilcox.csv", row.names = F)
write.csv(top10, "03_cellIdentify/top10_diff_genes_wilcox.csv", row.names = F)
# #专为单细胞设计的MAST
# diff.mast = FindAllMarkers(scRNA, test.use = 'MAST')
# all.markers = diff.mast %>% select(gene, everything()) %>% subset(p_val<0.05)
# top10 = all.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
# write.csv(all.markers, "03_cellIdentify/diff_genes_mast.csv", row.names = F)
# write.csv(top10, "03_cellIdentify/top10_diff_genes_mast.csv", row.names = F)
# #bulkRNA经典方法DESeq2
...
# top10 = all.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
# write.csv(all.markers, "03_cellIdentify/diff_genes_deseq2.csv", row.names = F)
# write.csv(top10, "03_cellIdentify/top10_diff_genes_deseq2.csv", row.names = F)
# ##top10基因绘制热图
# top10_genes <- read.csv("03_cellIdentify/top10_diff_genes_wilcox.csv")
...
# ggsave("03_cellIdentify/top10_markers.pdf", plot=plot1, width=10, height=8) 
# ggsave("03_cellIdentify/top10_markers.png", plot=plot1, width=10, height=8)
...
ggsave("03_cellIdentify/selectgenes_VlnPlot.png", p1, width=6 ,height=8)
#featureplot展示
p2 <- FeaturePlot(scRNA, features = select_genes, reduction = "tsne", label=T, ncol=2)
ggsave("03_cellIdentify/selectgenes_FeaturePlot.png", p2, width=8 ,height=12)
p3=p1|p2
ggsave("03_cellIdentify/selectgenes.png", p3, width=10 ,height=8)
